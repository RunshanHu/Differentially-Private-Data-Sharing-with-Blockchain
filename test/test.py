...
if __name__ == '__main__':
    ...
    tuples = "0.6, [avg,age], [sum,salary], \
                   ...
                   [avg,salary], [query id], \
                   [delete, age], [query, disease]"
    dataids = ['DataA' + str(i) for i in range(20)]
    duration = timeit.timeit(
        "write(chaincode_name, [str(id) for id in dataids, tuples], validate = True)",
        number = 20,
    ...
    tuples = "0.6, [avg,age], [sum,salary], \
                   ...
                   [avg,salary], [query id], \
                   [delete, age], [query, disease]"
    dataids = ['DataB' + str(i) for i in range(200)]
    duration = timeit.timeit(
        "write(chaincode_name, [str(id) for id in dataids, tuples], validate = True)",
        number = 200,
        setup = "from __main__ import write, chaincode_name, tuples, dataids")
    ...
    tuples = "0.6, [avg,age], [sum,salary], \
                   ...
                   [avg,salary], [query id], \
                   [delete, age], [query, disease]"
    dataids = ['DataC' + str(i) for i in range(2000)]
    duration = timeit.timeit(
        "write(chaincode_name, [str(id) for id in dataids, tuples], validate = True)",
        number = 2000,
        setup = "from __main__ import write, chaincode_name, tuples, dataids")
